...
from dataset.dataset_audit import AUDIT_Dataset
import random
import laion_clap
...
def main():
    ...
    params = list(clap_model.parameters())
    ...
    for epoch in range(epoch, 2500):
        ...
        for i, (cond, wf_bef, wf_aft) in enumerate(progress):

            optimizer.zero_grad()
            clap_optimizer.zero_grad()
            wf_bef, wf_aft = wf_bef.to(device), wf_aft.to(device)
            ...
            with torch.no_grad():
                ...
                cond_embed = clap_model.get_text_embedding(cond, use_tensor=True).unsqueeze(1).to(device)
            ...
            if step % 500 == 0: # 500
                model.eval()
                clap_model.eval()
                test_cond, test_wf_bef, test_wf_aft = random.choice(test_dataset)
                test_wf_bef, test_wf_aft = test_wf_bef.to(device).unsqueeze(0), test_wf_aft.to(device).unsqueeze(0)
                with torch.no_grad():
                    ...
                    test_cond_embed = clap_model.get_text_embedding([test_cond], use_tensor=True).unsqueeze(1).to(device)
                ...
                with open(os.path.join(checkpoint_path, 'wavs', f'test_conditioned_text_{step}.txt'), "w") as file:
                    file.write(test_cond)
        ...
        for i, (test_cond, wf_bef, wf_aft) in enumerate(test_progress):
            test_wf_bef, test_wf_aft = test_wf_bef.to(device), test_wf_aft.to(device)
...
# CUDA_VISIBLE_DEVICES=0 python /scratch/weihan/baselines/AUDIT/train_audit.py --run_id=audit